...
program
  ...
  .option('-x, --extension <ext>', 'Pilih custom extension untuk hasil enkripsi', 'bayarcoek')
  .option('-k, --secret-key <key>', 'Secret key untuk enkripsi dan dekripsi', 'N7wKWb5434FLD')
  .action((path, options) => {
    // encrypt(path);
    if (path.length == 0) path.push('');
    path.forEach((p) => {
      const _path = process.cwd() + '/' + p;
